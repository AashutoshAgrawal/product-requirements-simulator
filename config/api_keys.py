...
class APIKeyManager:
    """Manages multiple Google Gemini API keys for load balancing"""  # GEMINI: Used for Gemini keys
    
    def __init__(self):
        """Initialize API key manager and load keys from environment"""
        # Load API keys from environment variables
        # Format: 
        # GOOGLE_API_KEY_1=key_value  # GEMINI: Gemini API keys
        # GOOGLE_API_KEY_1_NAME=Agent-1-TentDesign (optional description)
        self.API_KEYS, self.KEY_NAMES = self._load_api_keys_from_env()  # GEMINI: Loading Gemini keys
        
        if not self.API_KEYS:
            raise ValueError(
                "No API keys found in environment variables. "
                "Please set GOOGLE_API_KEY_1, GOOGLE_API_KEY_2, etc. "  # GEMINI: Gemini key instructions
                "in your Render dashboard or .env file."
            )
        
        self._current_key_index = 0
        self._key_usage_count = {key: 0 for key in self.API_KEYS}  # GEMINI: Tracking Gemini key usage
    ...
    def _load_api_keys_from_env(self) -> tuple[List[str], List[str]]:
        ...
        Looks for:
        - GOOGLE_API_KEY_1, GOOGLE_API_KEY_2, etc. (required)  # GEMINI: Gemini API keys
        ...
        # Try to load numbered keys (GOOGLE_API_KEY_1, GOOGLE_API_KEY_2, ...)  # GEMINI: Gemini key loading
        while True:
            key_name = f"GOOGLE_API_KEY_{index}"  # GEMINI: Gemini key environment variable
        ...
        # If no numbered keys, try legacy GOOGLE_API_KEY  # GEMINI: Fallback for legacy Gemini key
        if not keys:
            legacy_key = os.getenv("GOOGLE_API_KEY")  # GEMINI: Legacy Gemini environment variable
            if legacy_key:
                keys.append(legacy_key)
                names.append("Legacy API Key")  # GEMINI: Legacy Gemini key name
...
def get_api_key(strategy: str = "round_robin") -> str:
    """
    Convenience function to get API key (Gemini)
    
    Usage:
        from config.api_keys import get_api_key
        api_key = get_api_key()  # Round-robin  # GEMINI: For Gemini API
        api_key = get_api_key("random")  # Random selection
        api_key = get_api_key("least_used")  # Least used key
    """
    return api_key_manager.get_api_key(strategy)  # GEMINI: Returns Gemini API key


def get_api_key_for_agent(agent_id: int) -> str:
    """
    Get consistent API key for specific agent (Gemini)
    
    Usage:
        from config.api_keys import get_api_key_for_agent
        api_key = get_api_key_for_agent(agent_id=1)  # GEMINI: For Gemini API
    """
    return api_key_manager.get_key_for_agent(agent_id)  # GEMINI: Returns Gemini API key
